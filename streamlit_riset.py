...
if menu == "Input Data":
    ...
    if "split_data" in st.session_state and st.session_state.split_data is not None:
        ...
        if st.button("Run XGBoost with Bayesian Optimization"):
            ...
            st.write("Best Parameters:", opt.best_params_)
            st.write("Training Accuracy:", opt.score(X_train, y_train))
            st.write("Test Accuracy:", opt.score(X_test, y_test))
    else:
